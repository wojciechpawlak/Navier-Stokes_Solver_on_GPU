...
int main(int argc, char *argv[])
{
	...
	int THREADS_PR_WORKGROUP;
	if (argc > 2 ? THREADS_PR_WORKGROUP = atoi(argv[2]) :
		THREADS_PR_WORKGROUP = MAX_THREADS_PR_WORKGROUP);
	...
	RHS_h	= RMATRIX(0, imax+1, 0, jmax+1); 
	FLAG_h	= (int *) malloc(datasize_int);

	copy_array_real_2d_to_1d(U,		U_h,	imax+2, jmax+2);
	copy_array_real_2d_to_1d(V,		V_h,	imax+2, jmax+2);
	copy_array_real_2d_to_1d(F,		F_h,	imax+2, jmax+2);
	copy_array_real_2d_to_1d(G,		G_h,	imax+2, jmax+2);
	...
	copy_array_real(RHS,	RHS_h,	imax+2, jmax+2);
	...
	//for (t=0.0, cycle=0; t < t_end; t+=delt, cycle++) {
		...
		/*-----------------------------------------------*/
		//COMP_RHS(F_h, G_h, RHS_h, FLAG_h, imax, jmax, delt, delx, dely);
